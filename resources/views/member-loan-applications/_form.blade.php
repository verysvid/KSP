...
>
    <div>
        ...
        <input
            type="text"
            value="{{ $member->branch?->code ? $member->branch->code . ' - ' . $member->branch->name : ($member->branch?->name ?? '-') }}"
            class="form-control"
            disabled
        >
        <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">
            Cabang mengikuti data keanggotaan Anda.
        </p>
    </div>
    ...
    <div>
        ...
        <input
            type="text"
            value="{{ $member->member_number }} - {{ $member->name }}"
            class="form-control"
            disabled
        >
        <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">
            Pengajuan otomatis dibuat atas nama Anda.
        </p>
    </div>
            ...
            <template x-if="selectedLoanType">
                <div>
                    ...
                    <div class="mt-1 text-xs text-slate-500 dark:text-slate-400">
                        Nilai bunga mengikuti jenis pinjaman yang dipilih.
                    </div>
                </div>
            </template>
            <template x-if="!selectedLoanType">
    ...
    <div>
        ...
        <template x-if="selectedLoanType">
            <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">
                Batas: <span x-text="amountLimitText"></span>
            </p>
        </template>
    ...
    <div>
        ...
        <template x-if="selectedLoanType">
            <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">
                Tenor jenis pinjaman: <span x-text="tenorLimitText"></span>
            </p>
        </template>
        @error('tenor_months')
            <p class="form-error">{{ $message }}</p>
    ...
    <div>
        ...
        <select id="due_day" name="due_day" required class="form-select">
        ...
        <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">
            Angsuran pertama diasumsikan jatuh tempo pada bulan setelah tanggal dasar simulasi.
        </p>
        @error('due_day')
            <p class="form-error">{{ $message }}</p>
        @enderror
    ...
    @push('scripts')
        <script>
            document.addEventListener('alpine:init', () => {
                Alpine.data('memberLoanForm', (config) => ({
                    amountRaw: '',
                    amountDisplay: '',
                    ...
                    get amountLimitText() {
                        ...
                        if (min !== null && max !== null) {
                            return 'Rp ' + this.formatCurrency(min) +
                                ' s/d Rp ' + this.formatCurrency(max);
                        }
